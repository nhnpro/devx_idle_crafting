	...
	internal class SampleFrameObservable<T> : OperatorObservableBase<T>
	{
		private class SampleFrame : OperatorObserverBase<T, T>
		{
			...
			public IDisposable Run()
			{
				...
				IDisposable disposable = Observable.IntervalFrame(parent.frameCount, parent.frameCountType).Subscribe(new SampleFrameTick(this));
		...
		public SampleFrameObservable(UniRx.IObservable<T> source, int frameCount, FrameCountType frameCountType)
