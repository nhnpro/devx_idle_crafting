...
namespace Dreamteck.Splines.Primitives
{
	public class Capsule : SplinePrimitive
	{
		...
		protected override void Generate()
		{
			...
			points[0].SetTangentPosition(points[0].position + Vector3.down * 2f * (Mathf.Sqrt(2f) - 1f) / 3f * radius);
			...
			points[2].SetTangentPosition(points[2].position + Vector3.up * 2f * (Mathf.Sqrt(2f) - 1f) / 3f * radius);
			points[3].position = Vector3.left / 2f * radius + Vector3.down * height * 0.5f;
			points[3].SetTangentPosition(points[3].position + Vector3.up * 2f * (Mathf.Sqrt(2f) - 1f) / 3f * radius);
			...
			points[5].SetTangentPosition(points[5].position + Vector3.down * 2f * (Mathf.Sqrt(2f) - 1f) / 3f * radius);
			points[6] = points[0];
		}
	}
}
