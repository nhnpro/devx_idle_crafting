...
public class MeshVertexAOForAR : MonoBehaviour
{
	private Mesh m_mesh;
	...
	protected void OnEnable()
	{
		...
		Singleton<AORunnerForAR>.Instance.Register(this);
	}

	protected void OnDisable()
	{
		if (Singleton<AORunnerForAR>.Instance != null)
		{
			Singleton<AORunnerForAR>.Instance.Unregister(this);
		}
	}
	...
	private float CalculateAO(Vector3 position, Vector3 normal)
	{
		...
		Vector3 position2 = a + a2 * ARInstaller.Instance.Scale / 2f;
		int num = Physics.OverlapSphereNonAlloc(position2, ARInstaller.Instance.Scale / 2f * 0.9f, m_buffer, m_raycastMask);
		for (int i = 0; i < num; i++)
		{
