...
public class UIPropertyCompareTriggerSetActive : UIAlwaysStartPropertyBase
{
	[SerializeField]
	...
	public override void AlwaysStart()
	{
		...
		(from val in property.TakeWhile((int _) => this != null)
			select ShouldBeActive(val) into active
			where active
			select active).SubscribeToActiveUntilNull(base.gameObject).AddTo(this);
	}
